"""
Train Engineer MLP V2 e5204 from 64x64 station images.
================================================

This training file connects the two modules already built:

    e5204_metrics_extractor.py
        image + valid mask -> combined vector [5204] = engineered1108 + spatial4096

    e5204_neural_network.py
        standardized 5204D -> segmentation logits [1,64,64]

Pipeline
--------
1. Discover station image / valid mask / GT mask pairs.
2. Extract deterministic 5204D vectors and cache them once.
...
# -----------------------------------------------------------------------------
# Deterministic 5204D feature cache
# -----------------------------------------------------------------------------
# Extraction is deterministic and more expensive than loading a 5204-float
# vector, so the training script caches each station once.
FEATURE_CACHE_DIR = Path("/home/hieu/Desktop/Nidec Vision/e5204_cacheche_5204_v1")
...
def main() -> None:
    ...
    print("=" * 78)
    print("Engineer MLP e5204 training")
    print("=" * 78)
