...
class CameraResourceController:
    ...
    def _coerce_parameter(value: Any, data_type: str) -> Any:
        if value is None:
            return None
        if data_type == 'number': return float(value)
        if data_type == 'integer': return int(value)
        if data_type == 'bool':
            if isinstance(value, str): return value.strip().lower() in {'1','true','on','yes'}
            return bool(value)
        return str(value)
    ...
    def call_custom(self, camera: CameraDeclaration, api: CameraCustomApi, args: dict[str, Any]) -> dict[str, Any]:
        ...
        values = {p.name: self._coerce_parameter(args.get(p.name, p.default), p.data_type) for p in api.parameters}
        placeholders = {name for _, name, _, _ in Formatter().parse(api.path_template) if name}
        missing = [name for name in placeholders if name not in values or values[name] is None]
        if missing:
            raise ValueError(f"Missing custom camera API parameter(s): {', '.join(missing)}")
        try:
            path = api.path_template.format(**values)
        except KeyError as exc:
            raise ValueError(f"Missing custom camera API parameter: {exc}") from exc
        # GET parameters that are declared but are not placeholders are appended as query params.
        if api.method == 'GET':
            extra = {k: v for k, v in values.items() if k not in placeholders and v is not None}
            if extra:
                path += ('&' if '?' in path else '?') + urlencode(extra)
