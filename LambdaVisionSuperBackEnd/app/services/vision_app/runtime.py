...
class VisionProgramRuntime:
    ...
    def _run_scope(
        ...
        *,
        ...
        # v0.9+: Decision ownership moves to the future IDE/sandbox glue layer.
        # Legacy v0.8/v2 programs keep their stored Decision DSL behaviour.
        legacy_decision = int(getattr(getattr(self, "_active_program", None), "version", 3)) < 3
        decision_enabled = bool(scope.enable_decision and legacy_decision)
        decision = DecisionExecutionResult(
            enabled=decision_enabled,
            ok=filter_ok and logic_ok,
            result="OK" if filter_ok and logic_ok else "NG",
            message="Reserved for future IDE/sandbox" if not decision_enabled else "",
        )
        if filter_ok and decision_enabled:
            d0 = perf_counter()
            try:
                ...
                decision = DecisionExecutionResult(enabled=True, ok=decision_ok, result=result_text, message="Legacy Decision script evaluated", elapsed_ms=(perf_counter() - d0) * 1000.0)
            except Exception as exc:
                decision = DecisionExecutionResult(enabled=True, ok=False, result="NG", message=str(exc), elapsed_ms=(perf_counter() - d0) * 1000.0)
            benchmarks.append(BenchmarkStep(scope=scope_id, phase="decision", name="Legacy Decision script", elapsed_ms=decision.elapsed_ms, detail=decision.message))

        scope_ok = filter_ok and (decision.ok if decision_enabled else logic_ok)
        return (
            ScopeRunResult(
                scope_id=scope_id,
    ...
    def preview_scope_filters(
        ...
        *,
        scope_id: str,
    ) -> tuple[str, list[Any], list[BenchmarkStep]]:
        run_id = self.debug.new_run()
        benchmarks: list[BenchmarkStep] = []
        if scope_id == "global":
            scope = program.working.global_scope
            preview_input = np.ascontiguousarray(test_image)
            self.debug.put(
                run_id,
                "preview_input",
                preview_input,
            ...
            artifact = _frame_from_array(preview_input, "vision_app:preview:global")
        else:
            ...
            if global_scope.enable_filter:
                global_artifact, _, _ = self._filter_stack(
                    global_scope.filter_services,
                    global_artifact,
                    scope_name="preview_global",
                    run_id=None,
                    benchmarks=None,
                    emit_debug=False,
                )
            ...
            located = {
                item.roi_id: item
                for item in locate_all(program.master.rois, filtered_master, filtered_test, program.master.locator)
            }
            ...
            preview_input = _crop_array(filtered_test, found.rect)
            self.debug.put(
                run_id,
                "preview_input",
                preview_input,
                label=f"{found.name} · Input",
                scope=scope_id,
                phase="input",
            )
            artifact = _frame_from_array(preview_input, f"vision_app:preview:{scope_id}")
        ...
        return run_id, self.debug.refs(run_id), benchmarks
