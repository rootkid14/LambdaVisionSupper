...
class FindCharucoExtrinsicsInput(BaseModel):
    ...
    intrinsics_report_file_path: str = Field(
        ...,
        title="Intrinsic Calibration Report TXT Path",
        description=UIDataType.STRING.value,
    )
...
class FindCharucoExtrinsicsNode(
    ...
    def _extract_report_section(
        ...
        """Return the text belonging to a named numbered report section."""
        ...
        if start_index is None:
            raise ValueError(
                "Intrinsic calibration report does not contain section: "
                + section_title
            )
    ...
    def _extract_first_json_object(
        ...
        if start < 0:
            raise ValueError(
                f"{section_name} does not contain a JSON object."
            )
        ...
        try:
            ...
            raise ValueError(
                f"Invalid JSON object in {section_name}: {error}"
            ) from error

        if not isinstance(value, dict):
            raise ValueError(
                f"{section_name} JSON value must be an object."
            )

        return value

    @staticmethod
    def _parse_report_boolean(
        report_text: str,
        label: str,
    ) -> Optional[bool]:
        pattern = re.compile(
            rf"^\s*{re.escape(label)}\s*:\s*(True|False)\s*$",
    ...
    @classmethod
    def _parse_intrinsics_report(
        cls,
        report_path: Path,
    ) -> Dict[str, Any]:
        """
        Parse the TXT report generated by the Intrinsics node.
        ...
        Optional section:
            DATASET SUMMARY -> image_size

        The report may say validation/overall_success=False. This parser does
        not reject such a profile automatically, because development/test
        workflows may deliberately use a non-production profile. The status is
        preserved in metadata so the engineer can see it in the Extrinsics
        output.
        """
        if not report_path.is_file():
            raise ValueError(
                "Invalid Intrinsic Calibration Report path: "
                + str(report_path)
            )
        ...
        if not report_text.strip():
            raise ValueError(
                "Intrinsic Calibration Report is empty."
            )

        board_section = cls._extract_report_section(
            report_text,
            "BOARD CONFIGURATION",
        )

        board_payload = cls._extract_first_json_object(
            board_section,
            "BOARD CONFIGURATION",
        )

        matrix_section = cls._extract_report_section(
            report_text,
            "CAMERA MATRIX K",
        )
        ...
        if len(matrix_rows) != 3:
            raise ValueError(
                "CAMERA MATRIX K must contain exactly three 3-value rows. "
                f"Found {len(matrix_rows)} valid rows."
            )

        distortion_section = cls._extract_report_section(
            report_text,
            "DISTORTION COEFFICIENTS D",
        )
        ...
        for line in distortion_section.splitlines():
            stripped = line.strip()
        ...
        if distortion_coefficients is None:
            raise ValueError(
                "DISTORTION COEFFICIENTS D does not contain "
                "a valid coefficient array."
            )

        image_size = None

        try:
            dataset_section = cls._extract_report_section(
                report_text,
                "DATASET SUMMARY",
            )

            dataset_summary = cls._extract_first_json_object(
                dataset_section,
                "DATASET SUMMARY",
            )

            size_value = dataset_summary.get(
                "image_size"
            )
            ...
            # image_size is useful metadata, but K/D/board parsing does not
            # depend on the DATASET SUMMARY section.
            image_size = None

        report_status = {
            "calibration_success": (
                cls._parse_report_boolean(
                    report_text,
                    "Calibration success",
                )
            ),
    ...
    def _extract_camera_profile(
        ...
        if camera_matrix_value is None:
            raise ValueError(
                "Camera profile does not contain "
                "camera_matrix or K."
            )
    ...
    def _detect_reference_image(
        ...
        #
        # K and D are loaded from the Intrinsics TXT report. The report image
        # size is kept as metadata, while this node still allows a different
        # reference-image size for development/debug workflows.
    ...
    async def execute(
        ...
        result: Dict[str, Any] = {
            ...
            "intrinsics_report_file": str(
                intrinsics_report_path.resolve()
            ),
            "intrinsics_report_status": {},
            "pose_success": False,
        ...
        try:
            ...
            image_size_matches_calibration = (
                None
                if calibration_image_size is None
                else tuple(calibration_image_size)
                == tuple(reference_image_size)
            )

            result["intrinsics"] = {
                "source": "INTRINSICS_REPORT_TXT",
                "source_report_file": str(
                    intrinsics_report_path.resolve()
                ),
                ...
                "usage_note": (
                    "K, D, and ChArUco board configuration "
                    "were loaded together from the same "
                    "Intrinsics TXT report."
                ),
            }
