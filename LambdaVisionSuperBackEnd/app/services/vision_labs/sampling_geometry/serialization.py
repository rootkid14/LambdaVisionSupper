...
def artifact_to_json(value: Any) -> dict[str, Any]:
    ...
    if isinstance(value, DataBlockSet):
        return {
            "type": "data_block_set",
            "count": len(value.blocks),
            "blocks": [
                {
                    ...
                    "values": np.asarray(block.values, dtype=np.float64).reshape(-1).tolist(),
    ...
    if isinstance(value, ComposedData):
        array = np.asarray(value.values, dtype=np.float64)
        return {
            "type": "composed_data",
            "shape": list(array.shape),
            "layout": value.layout,
            "block_order": list(value.block_order),
            "block_shapes": dict(value.block_shapes),
            "values": array.tolist(),
            "metadata": value.metadata,
        }
