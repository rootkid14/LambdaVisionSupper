...
class ImageOperatorRegistry:
    ...
    def hide_from_catalog(self, *operator_ids: str) -> None:
        self._catalog_hidden.update(str(operator_id) for operator_id in operator_ids)

    def show_in_catalog(self, *operator_ids: str) -> None:
        for operator_id in operator_ids:
            self._catalog_hidden.discard(str(operator_id))

    def manifests(self) -> list[dict[str, Any]]:
        return [
            definition.to_manifest()
            for definition in self.list()
            if definition.operator_id not in self._catalog_hidden
        ]

    def clear(self) -> None:
        self._definitions.clear()
        self._catalog_hidden.clear()
