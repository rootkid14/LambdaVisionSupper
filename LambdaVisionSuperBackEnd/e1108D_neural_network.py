...
Architecture:
    Specialized path -> 576D
    Holistic path    -> 576D
    concat           -> 1152D
    residual fusion  -> 512D
    residual reasoning x2 -> 512D
    latent           -> 256D
    coarse head      -> 16x16 logits
    fine head        -> 64x64 residual logits
    final logits     = upsample(coarse) + fine_residual

Notes:
- Standardize each of the 1108 input dimensions using TRAINING-SPLIT statistics.
- GT paste mask is never part of the 1108D input.
- Full 64x64 valid mask is used to mask the loss and may be applied to output at inference.
"""
...
class ResidualFusion(nn.Module):
    ...
    Inspired by Add & Norm:
        ...
        u = concat(specialized, holistic)      # 1152D
        main = MLP(u)                          # 512D
...
class EngineerMLP(nn.Module):
    def __init__(self, cfg: EngineerMLPConfig = EngineerMLPConfig()):
        ...
        if cfg.input_dim != 1108:
            raise ValueError(f"Expected 1108D input, config gives {cfg.input_dim}D")
        ...
        # Holistic path: sees the whole 1108D vector at once
        self.holistic_encoder = nn.Sequential(
            nn.Linear(cfg.input_dim, cfg.holistic_hidden_dim),
        ...
        # Dual-path fusion
        dual_dim = cfg.specialized_dim + cfg.holistic_embed_dim  # 1152
    ...
    def split_features(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        if x.ndim != 2:
            raise ValueError(f"Input must be [B, 1108], got {tuple(x.shape)}")
    ...
    def forward(self, x: torch.Tensor, return_intermediates: bool = False):
        ...
        # Path B: holistic encoder
        holistic = self.holistic_encoder(x)  # [B, 576]

        # Fusion
        dual = torch.cat([specialized, holistic], dim=1)  # [B, 1152]
        fused = self.fusion(dual)                         # [B, 512]
