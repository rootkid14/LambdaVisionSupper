"""
Entropy #1 - e5204 Station Feature Extractor
...
Output
------
5204D float32 feature vector in this exact order:
    Global       48D   [0:48]
    Grid        128D   [48:176]
    ...
    Spatial-L  4096D   [1108:5204]

IMPORTANT
---------
- GT paste mask is NEVER used to compute features.
- Invalid pixels are missing data, not intensity zero.
- Dimensions [0:1108] are the original engineered measurements.
- Dimensions [1108:5204] are the normalized 64x64 spatial L-map.
- Dataset per-dimension standardization is fitted using TRAIN split only.
"""
...
class FeatureConfig:
    ...
    # G. Spatial-L 64x64 -> 4096D
    spatial_eps: float = 1e-6
    spatial_clip: float = 5.0


CFG = FeatureConfig()

ENGINEERED_FEATURE_DIM = 1108
SPATIAL_FEATURE_DIM = 64 * 64
TOTAL_FEATURE_DIM = ENGINEERED_FEATURE_DIM + SPATIAL_FEATURE_DIM  # 5204
...
OUTPUT_ROOT = Path("_entropy1_features_5204")
...
def build_feature_names(cfg: FeatureConfig = CFG) -> List[str]:
    ...
    assert len(n) == ENGINEERED_FEATURE_DIM

    # G. Spatial-L 64x64 = 4096D, row-major ordering.
    for y in range(cfg.station_size):
        for x in range(cfg.station_size):
            n.append(f"spatial.Lz.y{y:02d}.x{x:02d}")

    assert len(n) == TOTAL_FEATURE_DIM
    return n
...
def _spatial_l_map(L: np.ndarray, valid: np.ndarray, cfg: FeatureConfig) -> np.ndarray:
    """Create the additional 4096D normalized 64x64 Lab-L morphology map."""
    vb = valid.astype(bool)
    spatial = np.zeros_like(L, dtype=np.float32)

    if not vb.any():
        return spatial.reshape(-1)

    values = L[vb].astype(np.float32)
    mu = float(values.mean())
    sigma = float(values.std())

    if sigma < cfg.spatial_eps:
        spatial[vb] = 0.0
    else:
        z = (L[vb] - mu) / (sigma + cfg.spatial_eps)
        spatial[vb] = np.clip(z, -cfg.spatial_clip, cfg.spatial_clip)

    out = spatial.reshape(-1).astype(np.float32)
    if out.shape != (SPATIAL_FEATURE_DIM,):
        raise RuntimeError(
            f"Expected {SPATIAL_FEATURE_DIM}D spatial map, got {out.shape}"
        )
    return out
...
def extract_feature_blocks(image: np.ndarray, valid_mask: np.ndarray, *, color_order: str="BGR", cfg: FeatureConfig=CFG):
    ...
    blocks={
        ...
        "spatial": _spatial_l_map(L, valid, cfg),
    }
    expected={
        "global":48,
        "grid":128,
...
def extract_feature_vector(image: np.ndarray, valid_mask: np.ndarray, *, color_order: str="BGR", cfg: FeatureConfig=CFG) -> np.ndarray:
    """Return float32 (5204,) = engineered1108 + spatial4096."""
    ...
    if v.shape!=(TOTAL_FEATURE_DIM,):
        raise RuntimeError(f"Expected {TOTAL_FEATURE_DIM}D, got {v.shape}")
...
def build_feature_dataset(image_dir: Path=IMAGE_DIR, valid_dir: Path=VALID_DIR, gt_dir: Optional[Path]=GT_DIR, output_root: Path=OUTPUT_ROOT):
    ...
    (output_root/"feature_names.json").write_text(json.dumps({"dim":TOTAL_FEATURE_DIM,"slices":{k:[s.start,s.stop] for k,s in FEATURE_SLICES.items()},"names":FEATURE_NAMES},indent=2),encoding="utf-8")
...
def sanity_check():
    ...
    assert v.shape==(TOTAL_FEATURE_DIM,) and len(FEATURE_NAMES)==TOTAL_FEATURE_DIM and np.isfinite(v).all()
