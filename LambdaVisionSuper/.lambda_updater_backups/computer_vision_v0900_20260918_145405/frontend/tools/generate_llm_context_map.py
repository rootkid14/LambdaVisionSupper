...
def _profiles() -> tuple[ModuleProfile, ...]:
    return (
        ...
        ModuleProfile(
            ...
            "Production Computer Vision application: Modbus trigger/result I/O, Camera acquisition (Basler or generic URL), program-wide Master ROI locator, Global/Station Filter→Logic→Decision orchestration, debug views and benchmarking.",
            (
                "exact:src/Pages/ComputerVisionPage.tsx",
                "prefix:src/components/ComputerVision/",
                "exact:src/api/visionAppApi.ts",
            ),
            "Computer Vision is the production orchestration layer. Trigger can drive Camera capture then inspection; Global filtering precedes ROI location and station scopes. Filter uses Image Processing services, Logic extracts data, and the built-in safe Decision DSL consumes logic outputs. LAB internals stay outside this module.",
            3,
        ),
        ModuleProfile(
            "fe_lab_service",
        ...
        ModuleProfile(
            "be_vision_app",
            "BE",
            "Computer Vision Program Backend",
...
BUNDLE_DEFINITIONS = {
    ...
    "Computer Vision Program": {
        "purpose": "Production Computer Vision application: Modbus trigger/result I/O, Basler/URL camera acquisition, stored Master Sample with one locator family for all stations, Global-first Filter→Logic→Decision execution, Local station scopes, debug previews and benchmark timings.",
        "minimal": (
...
def render_architecture(
    ...
    lines = [
        ...
        "  → Global Scope: sequential Filter services → Logic services → Decision DSL",
        "  → ROI location runs on the globally filtered image (master filtered equivalently)",
        "  → Local station scopes: Filter → Logic → Decision; sequential or parallel",
        "  → debug image store + per-step benchmark → overall OK/NG",
        "```",
        "",
        "### Lab Service",
        "",
