...
async def scope_preview(program_id: str, scope_id: str, request: Request):
    try:
        program = repository.get(program_id)
        run_id, refs = runtime.preview_scope_filters(program, repository.load_master(program_id), _decode(await request.body()), scope_id=scope_id)
        return {"success": True, "run_id": run_id, "debug_images": [model_to_dict(item) for item in refs]}
    except FileNotFoundError as exc:
        raise _error(exc, 404)
    except Exception as exc:
        raise _error(exc)
