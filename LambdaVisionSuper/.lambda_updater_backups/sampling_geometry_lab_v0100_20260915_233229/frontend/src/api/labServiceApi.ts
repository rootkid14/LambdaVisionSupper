...
import type { ImagePipelineDefinition } from '../components/VisionLabs/ImageProcessing/types';
...
export interface LabServiceDefinition {
  ...
  pipeline_snapshot: ImagePipelineDefinition | Record<string, any>;
...
export interface DeployLabServiceRequest {
  ...
  name: string;
  lab_type: 'image_processing' | string;
  pipeline_snapshot: ImagePipelineDefinition;
  outputs: Record<string, {
    node_id: string;
    port: string;
    label?: string | null;
