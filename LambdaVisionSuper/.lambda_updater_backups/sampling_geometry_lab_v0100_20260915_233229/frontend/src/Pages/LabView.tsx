      ...
      <div className="px-4 py-3 border-b border-[#3c4043] flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-0.5 truncate font-mono text-[9px] text-[#80868b]">
            {service.service_id} · {service.lab_type}
          </div>
        </div>
        <button
          onClick={onEdit}
          ...
          <section>
            ...
            {services.length === 0 ? (
              <div className="rounded-lg border border-dashed border-[#5f6368] bg-[#292a2d] p-8 text-center text-sm text-[#9aa0a6]">
                No Lab Service has been deployed yet. Open Image Processing LAB, expose checkpoints, then deploy a service.
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {services.map((service) => (
                  <LabServiceCard
                    ...
                    service={service}
                    onEdit={() => navigate(`/labs/image-processing?editService=${encodeURIComponent(service.service_id)}`)}
