...
_ALLOWED_NODE_TYPES = (
    ...
    ast.Attribute, ast.Call, ast.Compare, ast.BoolOp, ast.UnaryOp, ast.BinOp,
...
class AutomationExecutionContext:
    ...
    call_action: Callable[[str, list[Any], bool], Any]
...
class SafeAutomationInterpreter:
    ...
    def _expr(self, node: ast.AST) -> Any:
        ...
        if isinstance(node, ast.Call):
            ...
            if path:
                value = self.ctx.call_action(path, args, self.ctx.dry_run)
                self._trace("Call action", endpoint=path, value=args)
                return value
