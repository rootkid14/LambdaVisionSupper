...
class AutomationManager:
    ...
    def state(self, program_id: str) -> AutomationSystemState:
        with self._lock:
            state = self._states.setdefault(program_id, AutomationSystemState(program_id=program_id))
            try:
                program = self.repository.get(program_id)
                if not state.active_workspace:
                    state.active_workspace = safe_alias(workspace_by_alias(program, program.active_workspace_id).alias, 'workspace_1')
            except Exception:
                pass
            return state
    ...
    def activate_workspace(self, program_id: str, workspace_alias: str) -> None:
        program = self.repository.get(program_id)
        workspace = workspace_by_alias(program, workspace_alias)
        self.state(program_id).active_workspace = safe_alias(workspace.alias, workspace.workspace_id)
        ...
        if camera_path is not None:
            ...
            for custom in camera.custom_apis:
                if custom.enabled and safe_alias(custom.alias, custom.api_id) == custom_alias:
                    ...
                    values = {p.name: (args[i] if i < len(args) else p.default) for i, p in enumerate(custom.parameters)}
