    ...
    def _declared_cameras(self, program: VisionProgramDefinition, specs: list[EndpointSpec], state: Any | None) -> None:
        ...
        for camera in program.cameras.devices:
            ...
            for custom in camera.custom_apis:
                ...
                specs.append(EndpointSpec(path=f"{root}.{api_alias}", kind="action", callable=True, readable=False, data_type="action", description=custom.description or f"HTTP {custom.method} {custom.path_template}", source=root, example=f"{root}.{api_alias}({example_args})", object_type="camera_custom_api"))
