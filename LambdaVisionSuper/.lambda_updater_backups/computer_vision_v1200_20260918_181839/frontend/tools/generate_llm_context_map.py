...
def _profiles() -> tuple[ModuleProfile, ...]:
    return (
        ...
        ModuleProfile(
            ...
            "Production Computer Vision application with Automation IDE control plane: typed Endpoint Registry, safe DSL, Loop/Event/One-shot services, execution trace/dry-run, Modbus I/O, Camera acquisition, Master ROI locator, Global/Station Filter→Logic orchestration, expandable scope controls, unified lazy Debug Stack Workspace and benchmarking.",
            (
                "exact:src/Pages/ComputerVisionPage.tsx",
                "prefix:src/components/ComputerVision/",
                "exact:src/api/visionAppApi.ts",
            ),
            "Computer Vision is the production orchestration layer. v0.11 adds an Automation control plane: Endpoint Registry exposes system/device/camera/vision state, actions, events and run data; safe Loop/Event/One-shot services consume those endpoints without Python exec; VisionRunSnapshot freezes Logic outputs for deterministic automation; Dry Run and Execution Trace support commissioning. Global filtering still precedes ROI location and station scopes; Debug Stack remains observational and lazy.",
            3,
        ),
        ModuleProfile(
            "fe_lab_service",
...
BUNDLE_DEFINITIONS = {
    ...
    "Computer Vision Program": {
        "purpose": "Production Computer Vision application with v0.11 Automation IDE/control plane: Endpoint Registry, safe DSL, Loop/Event/One-shot scheduler, VisionRunSnapshot, Dry Run and Execution Trace; plus Modbus I/O, Basler/URL camera, Master ROI localization, Global-first Filter→Logic execution, Local scopes and lazy Debug Stack.",
        "minimal": (
...
def render_architecture(
    ...
    lines = [
        ...
        "4. **Computer Vision is an orchestration/application layer.** It binds Modbus I/O, Camera, Master Sample/ROI station localization and deployed Lab Services; v0.11 Automation IDE is the control/glue plane over typed endpoints and must not absorb LAB algorithms.",
        ...
        "  → Endpoint Registry exposes system / camera / device.modbus_1 / vision namespaces",
        "  → Automation IDE: Loop / Event / One-shot services over a restricted AST DSL (no exec/import/while)",
