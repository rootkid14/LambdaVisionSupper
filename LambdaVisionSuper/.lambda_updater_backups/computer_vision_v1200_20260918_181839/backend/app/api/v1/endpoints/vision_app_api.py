...
async def test_run(program_id: str, request: Request):
    try:
        ...
        result = automation_manager.run_inspection(program_id, _decode(await request.body()))
...
def automation_latest_frame(program_id: str):
    try:
        repository.get(program_id)
        return Response(content=automation_manager.latest_frame_jpeg(program_id), media_type="image/jpeg")
    except FileNotFoundError as exc:
        raise _error(exc, 404)
    except Exception as exc:
        raise _error(exc)
...
async def automation_set_latest_frame(program_id: str, request: Request):
    try:
        repository.get(program_id)
        image = _decode(await request.body())
        automation_manager.set_latest_frame(program_id, image, announce=False)
