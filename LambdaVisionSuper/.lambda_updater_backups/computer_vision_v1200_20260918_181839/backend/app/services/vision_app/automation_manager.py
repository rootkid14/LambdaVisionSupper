...
class AutomationManager:
    ...
    def _snapshot_from_run(self, program: VisionProgramDefinition, run) -> VisionRunSnapshot:
        ...
        roi_name = {roi.roi_id: _safe_alias(roi.name, roi.roi_id) for roi in program.master.rois}
    ...
    def _read_path(self, program: VisionProgramDefinition, event: dict[str, Any], path: str) -> Any:
        ...
        if path == "device.modbus_1.X1": return self.io.read_point(program.io, program.io.trigger_kind, program.io.trigger_address)
        if path == "device.modbus_1.Y1": return self.io.read_point(program.io, "coil", program.io.ok_coil)
        if path == "device.modbus_1.Y2": return self.io.read_point(program.io, "coil", program.io.ng_coil)
        ...
        if path.startswith("vision.") and snap is not None:
            ...
            elif len(parts) >= 6 and parts[:2] == ["vision", "roi"] and parts[3] == "logic":
                data = snap.roi_logic.get(parts[2], {}).get(parts[4], {})
    ...
    def _write_path(self, program: VisionProgramDefinition, path: str, value: Any, dry_run: bool) -> None:
        if path == "device.modbus_1.Y1":
            if not dry_run: self.io.set_coil(program.io, program.io.ok_coil, bool(value))
            return
        if path == "device.modbus_1.Y2":
            if not dry_run: self.io.set_coil(program.io, program.io.ng_coil, bool(value))
            return
    ...
    def _call_action(self, program: VisionProgramDefinition, path: str, args: list[Any], dry_run: bool) -> Any:
        ...
        if path == "system.clear_working_screen":
            ...
            return True
        if path in {"device.modbus_1.Y1.pulse", "device.modbus_1.Y2.pulse"}:
            seconds = float(args[0]) if args else program.io.pulse_seconds
            ok = path.endswith("Y1.pulse")
            if not dry_run: self.io.pulse_result(program.io, ok=ok, seconds=seconds)
            return True
