...
class ModbusIOController:
    ...
    def read_point(self, config: ModbusIOConfig, kind: str, address: int) -> bool:
        """Read one declared Modbus boolean point for the Automation Endpoint Registry."""
        client = self._client(config)
        try:
            if not client.connect():
                raise ConnectionError(f"Modbus connection failed: {config.host}:{config.port}")
            if kind == "coil":
                response = self._device_call(client.read_coils, int(address), count=1, device_id=config.device_id)
            elif kind == "discrete_input":
                response = self._device_call(client.read_discrete_inputs, int(address), count=1, device_id=config.device_id)
            else:
                raise ValueError(f"Unsupported Modbus point kind: {kind}")
