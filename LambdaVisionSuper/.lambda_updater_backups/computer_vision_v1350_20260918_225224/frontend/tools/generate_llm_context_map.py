...
def _profiles() -> tuple[ModuleProfile, ...]:
    return (
        ...
        ModuleProfile(
            ...
            "Production Computer Vision application with v0.13.4 multi-resource model: IOT Declaration Engine, Camera Declaration Engine, strictly isolated multi-Workspace inspection programs with one-camera binding, typed dynamic Endpoint Registry, colored Automation IDE, Online/Offline runtime, Master ROI locator, Filter→Logic orchestration and lazy Debug Stack.",
            (
                "exact:src/Pages/ComputerVisionPage.tsx",
                "prefix:src/components/ComputerVision/",
                "exact:src/api/visionAppApi.ts",
            ),
            "Computer Vision is the production orchestration layer. v0.13.4 treats each Workspace as an independent inspection program: its own Master image, ROI/locator definition, Working filter/logic scopes, debug/run context and one-camera binding. Camera/IOT resources remain program-wide; workspace state must never fall back to another workspace.",
            3,
        ),
        ModuleProfile(
            "fe_lab_service",
...
BUNDLE_DEFINITIONS = {
    ...
    "Computer Vision Program": {
        "purpose": "Production Computer Vision application with v0.13.4 declared resources and strictly isolated multi-Workspace control plane: IOT + Camera Declaration Engines, one-camera-per-workspace binding, dynamic typed Endpoint Registry, colored Automation IDE/IntelliSense, Online/Offline scheduler, VisionRunSnapshot, per-workspace Master/ROI/Working state, Filter→Logic execution and lazy Debug Stack.",
        "minimal": (
