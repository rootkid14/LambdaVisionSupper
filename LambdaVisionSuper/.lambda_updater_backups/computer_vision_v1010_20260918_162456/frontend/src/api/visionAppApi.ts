...
export const VisionAppAPI = {
  ...
  previewScope: async (id:string,scopeId:string,file:Blob) => (await axiosClient.post(`${api_version}/computer-vision/programs/${encodeURIComponent(id)}/scope-preview/${encodeURIComponent(scopeId)}`,file,rawBody(file))).data as {run_id:string;debug_images:DebugImageRef[];benchmarks:BenchmarkStep[]},
