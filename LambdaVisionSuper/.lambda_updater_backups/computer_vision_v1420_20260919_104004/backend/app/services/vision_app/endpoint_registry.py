...
class EndpointRegistry:
    ...
    def _legacy_vision_aliases(self, program: VisionProgramDefinition, snapshot: VisionRunSnapshot | None, specs: list[EndpointSpec], state: Any | None) -> None:
        """v0.12 compatibility namespace projected from the active/latest workspace.

        Never read the mirrored program.master/program.working root directly here:
        those fields intentionally mirror Workspace 1 only for old integrations.
        """
        target = getattr(state, "last_workspace", None) or getattr(state, "active_workspace", None) or program.active_workspace_id
        workspace = None
        for candidate in program.workspaces:
            alias = safe_alias(candidate.alias, candidate.workspace_id)
            if candidate.workspace_id == target or alias == target:
                workspace = candidate
                break
        if workspace is None:
            workspace = program.workspaces[0] if program.workspaces else None
        if workspace is None:
            return
        specs.extend([
    ...
    def build(self, program: VisionProgramDefinition, snapshot: VisionRunSnapshot | None = None, state: Any | None = None) -> list[EndpointSpec]:
        ...
        self._legacy_vision_aliases(program, snapshot, specs, state)
        specs.extend([
