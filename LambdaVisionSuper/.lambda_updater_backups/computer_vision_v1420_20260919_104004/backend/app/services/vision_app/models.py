...
class SoftTriggerConfig(BaseModel):
    """Backend stream trigger owned by one inspection workspace.

    The trigger evaluates only a normalized ROI from the camera Streaming_frame.
    A rising threshold event snapshots the stream frame into Current_image and can
    immediately run that workspace. It rearms only after the active-pixel count
    falls below reset_pixel_count, preventing repeated triggers while one product
    remains inside the sensor ROI.
    """
...
class WorkspaceDefinition(BaseModel):
    ...
    enabled: bool = True
    # v0.13.6 strict ownership: declarations belong to the Workspace itself.
    # Empty factories are intentional: a newly created Workspace must not inherit
    # Camera/IOT declarations or an image binding from another Workspace.
    iot: IotDeclarationConfig = Field(default_factory=lambda: IotDeclarationConfig(devices=[]))
    cameras: CameraDeclarationConfig = Field(default_factory=lambda: CameraDeclarationConfig(devices=[]))
    camera_id: str = ""
    input_binding: str = ""
...
class VisionProgramDefinition(BaseModel):
    ...
    name: str
    description: str = ""
    # Legacy root contracts remain for old integrations. v0.13.6 UI/runtime own IOT/Camera declarations per Workspace.
