...
def _profiles() -> tuple[ModuleProfile, ...]:
    return (
        ...
        ModuleProfile(
            ...
            "Consumes raster sources directly or through Lab Services. The editor scope is Spatial + Spectral only: Spatial answers WHERE + WHAT/HOW; Spectral/Statistics represents data with graphs, FFT reconstruction and local-frequency maps. Legacy geometry runtime remains backend-compatible only for old snapshots.",
            5,
        ),
        ModuleProfile(
            "fe_lab_service",
        ...
        ModuleProfile(
            ...
            "Feature-extraction backbone. Spatial owns independent sampling units and local data composition; Spectral/Statistics owns interpretable statistical/frequency representations. It does not own contour selection, OK/NG, or model training semantics.",
            4,
        ),
        ModuleProfile(
            "be_lab_service",
