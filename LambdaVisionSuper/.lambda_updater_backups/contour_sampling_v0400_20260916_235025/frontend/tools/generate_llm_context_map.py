...
def _profiles() -> tuple[ModuleProfile, ...]:
    return (
        ...
        ModuleProfile(
            ...
            "Memory-efficient contour candidate generation, early filtering, filter funnel, lazy geometry inspection, and Contour Extractor service deployment.",
            (
                "exact:src/Pages/ContourExtractorLabPage.tsx",
                "exact:src/api/contourExtractorApi.ts",
            ),
            "Owns contour discovery/selection. It produces meaningful contour sets rather than arbitrary measurements.",
            4,
        ),
        ModuleProfile(
            ...
            "FE",
            "Sampling LAB UI",
            "Spatial Sampling Units and spectral/statistical feature extraction with semantic Data Blocks and local layout composition.",
            (
            ...
            "Consumes raster sources directly or through Lab Services. Legacy geometry runtime remains backend-compatible, but new editor scope is Spatial + Spectral only.",
            5,
        ),
        ModuleProfile(
            "fe_lab_service",
        ...
        ModuleProfile(
            ...
            "Early-filtered contour candidate runtime with single-copy ContourStore, ID-only filter selections, lazy geometry access, sessions, and REST API.",
            (
                "exact:app/api/v1/endpoints/contour_extractor_api.py",
                "prefix:app/services/vision_labs/contour_extractor/",
            ),
            "Contour geometry is stored once; filter stages retain contour IDs rather than cloning coordinate arrays.",
            3,
        ),
        ModuleProfile(
            ...
            "Spatial Sampling Unit and spectral/statistical runtime, typed Data Blocks/composed data, visualization serialization, and Sampling REST API. Legacy geometry operators remain for old service snapshots.",
            (
                "exact:app/api/v1/endpoints/sampling_geometry_api.py",
                "prefix:app/services/vision_labs/sampling_geometry/",
            ),
            "Feature-extraction backbone. New Sampling editor scope is Spatial + Spectral; it does not own contour selection, OK/NG, or model training semantics.",
            4,
        ),
        ModuleProfile(
            "be_lab_service",
...
BUNDLE_DEFINITIONS = {
    ...
    "Sampling LAB": {
        "purpose": "Independent Spatial Sampling Units plus Fourier/statistical analysis, semantic Data Blocks, local layout composition, and reusable feature-extraction services.",
        "minimal": (
...
def render_architecture(
    ...
    lines = [
        ...
        "  → DEFAULT EARLY FILTER (reject geometry before store)",
        "  → ContourStore (single geometry copy)",
        "  → ID-only filter selections",
        "  → meaningful ContourSet",
        "```",
        "",
        "### Sampling LAB",
        ...
        "  → Spatial Sampling Units: Housing + Data Extractor + Data Blocks",
        "  → or Spectral/Statistics analysis",
        "  → local Data Layout Composer",
