...
export const SamplingCanvas = ({
  ...
  const overlayGeometries = overlayArtifacts.map(artifactGeometry).filter(Boolean);
  ...
  const overlay = (
    <svg viewBox={`0 0 ${imageSize.w} ${imageSize.h}`} className="absolute inset-0 h-full w-full overflow-visible">
      ...
      {overlayGeometries.map((g:any, gi:number) => <g key={`geometry:${gi}`}>
        {g?.kind === 'rays' ? (g.rays ?? []).map((ray:number[], index:number)=><line key={`r:${index}`} x1={ray[0]*imageSize.w} y1={ray[1]*imageSize.h} x2={ray[2]*imageSize.w} y2={ray[3]*imageSize.h} stroke={gi%2?'#81c995':'#8ab4f8'} strokeWidth="2" vectorEffect="non-scaling-stroke"/>) : null}
        {g?.kind === 'rings' ? (g.radii ?? []).map((radius:number,index:number)=>{const scale=Math.min(imageSize.w,imageSize.h);return <circle key={`c:${index}`} cx={(g.center?.[0]??0.5)*imageSize.w} cy={(g.center?.[1]??0.5)*imageSize.h} r={radius*scale} fill="none" stroke={gi%2?'#81c995':'#8ab4f8'} strokeWidth="2" vectorEffect="non-scaling-stroke"/>;}) : null}
        {g?.kind === 'boxes' ? (g.boxes ?? []).map((box:number[],index:number)=><rect key={`b:${index}`} x={box[0]*imageSize.w} y={box[1]*imageSize.h} width={(box[2]-box[0])*imageSize.w} height={(box[3]-box[1])*imageSize.h} fill="none" stroke={gi%2?'#81c995':'#8ab4f8'} strokeWidth="1.5" vectorEffect="non-scaling-stroke"/>) : null}
      </g>)}
    </svg>
  );
