...
def _profiles() -> tuple[ModuleProfile, ...]:
    return (
        ...
        ModuleProfile(
            ...
            (
                "exact:src/Pages/ContourExtractorLabPage.tsx",
                "exact:src/Pages/ContourExtractorGuideModal.tsx",
                "exact:src/Pages/ContourStageLibrary.tsx",
                "exact:src/Pages/ContourStageGuideModal.tsx",
                "exact:src/Pages/ContourFourierInspector.tsx",
                "exact:src/api/contourExtractorApi.ts",
            ),
            "Owns contour discovery/selection. Cheap memory gates run before retained geometry; advanced registered stages (including Fourier/master-shape primitives) operate on stable contour IDs and produce meaningful ContourSets rather than arbitrary measurements.",
            4,
        ),
        ModuleProfile(
        ...
        ModuleProfile(
            ...
            "Production Computer Vision application with v0.13 multi-resource model: IOT Declaration Engine, Camera Declaration Engine, multi-Workspace inspection contexts with camera frame-slot bindings, typed dynamic Endpoint Registry, colored Automation IDE, Online/Offline program runtime, Developer Simulator, Master ROI locator, Filter→Logic orchestration and lazy Debug Stack.",
            (
                "exact:src/Pages/ComputerVisionPage.tsx",
                "prefix:src/components/ComputerVision/",
                "exact:src/api/visionAppApi.ts",
            ),
            "Computer Vision is the production orchestration layer. v0.13 extends the object-driven control plane to declared cameras and inspection workspaces. Camera resources expose capture/stream slots/actions; each workspace owns Master/Working configuration and binds to an image slot. IOT/camera/workspace/ROI/Logic declarations immediately become Endpoint Registry objects; runtime snapshots enrich them with values.",
            3,
        ),
        ModuleProfile(
            "fe_lab_service",
        ...
        ModuleProfile(
            ...
            (
                "exact:app/api/v1/endpoints/contour_extractor_api.py",
                "prefix:app/services/vision_labs/contour_extractor/",
            ),
            "Contour geometry is stored once; the mandatory memory gate discards junk coordinates before the store, while registered filter/shape stages retain contour IDs rather than cloning geometry.",
            3,
        ),
        ModuleProfile(
            ...
            "Feature-extraction backbone. The new domain model is Image → Global/Local domain → placement/sampling shape → channels → measures → composed numerical shape. It does not own contour selection, frequency-domain transforms, OK/NG, or model training semantics.",
            4,
        ),
        ModuleProfile(
            "be_vision_app",
            ...
            (
                "exact:app/api/v1/endpoints/vision_app_api.py",
                "prefix:app/services/vision_app/",
                "prefix:tests/vision_app/",
            ),
            "Owns production-program orchestration only. Image Processing services form sequential filter stacks; logic services are independent extractors; a restricted decision DSL produces scope OK/NG. ROI stations may run sequentially or in parallel. Future Rule/AI services can replace/extend the built-in decision frame.",
            3,
        ),
        ModuleProfile(
            "be_lab_service",
...
BUNDLE_DEFINITIONS = {
    ...
    "Contour Extractor LAB": {
        "purpose": "Memory-efficient contour discovery: candidate generation, early junk rejection, filter funnel, lazy geometry inspection, and deployable ContourSet services.",
        "minimal": (
    ...
    "Sampling LAB": {
        ...
        "deep": (
            ("FE", "src/components/VisionLabs/SamplingProgram/"),
    ...
    "Computer Vision Program": {
        "purpose": "Production Computer Vision application with v0.13 declared resources and multi-Workspace control plane: IOT + Camera Declaration Engines, camera frame slots, workspace input bindings, dynamic typed Endpoint Registry, colored Automation IDE/IntelliSense, Online/Offline scheduler, Developer Simulator, VisionRunSnapshot, Master ROI localization, Filter→Logic execution and lazy Debug Stack.",
        "minimal": (
...
def render_architecture(
    ...
    lines = [
        ...
        "4. **Computer Vision is an orchestration/application layer.** IOT and Camera are declared resources; Workspaces are inspection contexts with their own Master/Working configuration and input bindings. Automation IDE is the glue plane over typed endpoints and must not absorb LAB algorithms.",
        ...
        "  → Camera Declaration Engine: Basler / HTTP / simulated cameras expose capture, stream slots and custom APIs
  → Workspaces: multiple independent Master/Working contexts bound to camera image slots
  → Automation IDE: colored safe-DSL editor + registry-backed dot completion + hierarchical Object/Endpoint Explorer
  → Online/Offline: arm/disarm all Loop/Event automation at program level
  → Developer Simulator: simulated IOT points and simulated camera frames use the same endpoint objects",
