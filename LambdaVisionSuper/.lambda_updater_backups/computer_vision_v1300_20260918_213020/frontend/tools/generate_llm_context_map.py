...
def _profiles() -> tuple[ModuleProfile, ...]:
    return (
        ...
        ModuleProfile(
            ...
            "Production Computer Vision application with v0.12 live object model: IOT Declaration Engine, typed dynamic Endpoint Registry, colored Automation IDE with object-tree IntelliSense, safe DSL, Loop/Event/One-shot services, execution trace/dry-run, Camera acquisition, Master ROI locator, Global/Station Filter→Logic orchestration and lazy Debug Stack.",
            (
                "exact:src/Pages/ComputerVisionPage.tsx",
                "prefix:src/components/ComputerVision/",
                "exact:src/api/visionAppApi.ts",
            ),
            "Computer Vision is the production orchestration layer. v0.12 makes the control plane object-driven: IOT devices/points, ROI aliases and Logic aliases are declarations that immediately become typed Endpoint Registry objects, even before a run. Runtime snapshots enrich those objects with values/output leaves. The Automation IDE uses the same live graph for colored IntelliSense and its object explorer.",
            3,
        ),
        ModuleProfile(
            "fe_lab_service",
...
BUNDLE_DEFINITIONS = {
    ...
    "Computer Vision Program": {
        "purpose": "Production Computer Vision application with v0.12 live object control plane: IOT Declaration Engine, dynamic typed Endpoint Registry, colored object-aware Automation IDE/IntelliSense, safe Loop/Event/One-shot scheduler, VisionRunSnapshot, Dry Run and Execution Trace; plus Basler/URL camera, Master ROI localization, Global-first Filter→Logic execution and lazy Debug Stack.",
        "minimal": (
