...
export interface AutomationEndpoint {
  ...
  description:string; source:string; example:string; last_value:any;
}
...
export interface AutomationSystemState {
  program_id:string; scheduler_running:boolean; run_state:'IDLE'|'CAPTURING'|'INSPECTING'|'DECIDING'|'FINISHED'|'ERROR';
  result:'NONE'|'OK'|'NG'|'ERROR'; run_id:string; last_run_ms?:number|null; last_error:string;
  frame_sequence:number; clear_sequence:number; result_sequence:number; last_event:string;
}
...
export interface ModbusDeviceDeclaration {
  declaration_id:string; alias:string; enabled:boolean; driver:'modbus_tcp'; host:string; port:number; unit_id:number; points:ModbusPointDeclaration[];
}
...
export const VisionAppAPI = {
  ...
  previewScope: async (id:string,scopeId:string,file?:Blob|null) => {
    const url=`${api_version}/computer-vision/programs/${encodeURIComponent(id)}/scope-preview/${encodeURIComponent(scopeId)}`;
    const response=file
      ? await axiosClient.post(url,file,rawBody(file))
      : await axiosClient.post(url,null,{timeout:120000});
  ...
  automationLatestFrame: async (id:string):Promise<Blob> => (await axiosClient.get(`${api_version}/computer-vision/programs/${encodeURIComponent(id)}/automation/latest-frame`,{responseType:'blob'})).data,
  automationSetLatestFrame: async (id:string,file:Blob) => (await axiosClient.post(`${api_version}/computer-vision/programs/${encodeURIComponent(id)}/automation/latest-frame`,file,rawBody(file))).data,
};
