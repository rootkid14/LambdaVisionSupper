...
def _profiles() -> tuple[ModuleProfile, ...]:
    return (
        ...
        ModuleProfile(
            ...
            "FE",
            "Sampling LAB UI",
            "Hierarchical Global/Local Sampling Program editor: whole-image measurements, patch-grid local recipes, spatial sampling shapes, channel/measure configuration, image-grounded teaching, and Output Shape Designer for vectors/matrices/spatial tensors.",
            (
            ...
            "Consumes one inspected raster source directly or through Image Processing Lab Services. New editor/runtime semantics are SamplingProgram(Global, Local Grid, methods, channels/measures, output shape); FFT/frequency transforms are intentionally out of editor scope. Legacy v0.1-v0.4 pipelines remain runtime-compatible only for old snapshots.",
            5,
        ),
        ModuleProfile(
            "fe_lab_service",
        ...
        ModuleProfile(
            ...
            "SamplingProgram runtime with Global whole-image methods, Local patch-grid replication, spatial shapes (rays/cross/rings), channel/statistical extraction, semantic Data Blocks, composed output shapes, image-grounded channel previews, and Sampling REST/service adapters. Legacy pipeline/spectral code remains only for old service snapshots.",
            (
...
BUNDLE_DEFINITIONS = {
    ...
    "Sampling LAB": {
        "purpose": "Hierarchical Global/Local Sampling Program that turns one inspected image into understandable vectors, patch matrices, or spatial tensors without exposing frequency-domain transforms in the current editor.",
        "minimal": (
...
def render_architecture(
    ...
    lines = [
        ...
        "  → Output Shape Designer: vector / patch matrix / spatial tensor",
        "  → SamplingProgramRuntime → composed_data Lab Service outputs",
