...
# ---------------------------------------------------------------------------
# Sampling Program v0.5 — hierarchical Global / Local editor runtime.
# Legacy pipeline endpoints above remain available for deployed v0.1-v0.4
# snapshots; the new editor no longer builds a linear operator stack.
# ---------------------------------------------------------------------------
...
@router.post(
    ...
    summary="Run the hierarchical Sampling Program on the inspected image",
)
def run_sampling_program(
    ...
    try:
        ...
        result = SamplingProgramRuntime(program).run(value)
        return {
            "success": True,
            ...
            "local": {
                ...
                "patches": [
                    artifact_to_json(patch)
                    for patch in result.local_patch_blocks
                ],
            },
