...
export interface SourceBoardServicePin {
  service_id: string;
  version?: number | null;
  output_name: string;
}
...
export interface SourceBoardEntry {
  name: string;
  label: string;
  type: string;
  shape?: number[] | null;
...
export interface SourceBoardManifest {
  config: SourceBoardConfig;
  upstream?: Record<string, any>;
  entries: SourceBoardEntry[];
  geometry_contour_count?: number;
}
...
export interface SamplingStackItem {
  ...
  exposedOutputs: Record<string, string>;
}

export interface ContourMetric {
  id: number;
  point_count: number;
  area_px2: number;
  perimeter_px: number;
  open_length_px?: number;
  bbox: number[];
  centroid: number[];
  aspect_ratio: number;
  circularity: number;
...
export type SamplingArtifact =
  ...
  | { type: 'contour_set'; source_shape: number[]; contours: number[][][]; contour_ids?: number[]; metrics?: ContourMetric[]; count?: number; metadata?: any }
  | { type: 'polyline'; points: number[][]; closed: boolean; source_shape?: number[] | null; point_count?: number; arc_length_px?: number; metadata?: any }
...
export interface SharedInputState {
  file: File | null;
