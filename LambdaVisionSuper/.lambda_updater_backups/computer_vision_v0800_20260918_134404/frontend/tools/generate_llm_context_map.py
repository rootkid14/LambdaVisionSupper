...
def _profiles() -> tuple[ModuleProfile, ...]:
    return (
        ...
        ModuleProfile(
            ...
            "Production-oriented Computer Vision program frame: Modbus I/O, Master Sample/ROI station localization, and Global/Local Working service orchestration.",
            (
                "exact:src/Pages/ComputerVisionPage.tsx",
                "prefix:src/components/ComputerVision/",
                "exact:src/api/visionAppApi.ts",
            ),
            "Computer Vision orchestrates deployed Lab Services and station-level program semantics. It should not duplicate Image Processing, Contour Extractor, Sampling, Rule or AI internals.",
            3,
        ),
        ModuleProfile(
            "fe_lab_service",
        ...
        ModuleProfile(
            ...
            "Persisted Vision Program definitions, Modbus I/O contract, Master Sample/ROI localization, and Global/Local station execution through LabServiceRuntime.",
            (
                "exact:app/api/v1/endpoints/vision_app_api.py",
                "prefix:app/services/vision_app/",
                "prefix:tests/vision_app/",
            ),
            "Owns production-program orchestration only. Global/Local working stages call versioned Lab Services directly; Comparison/Rule and AI semantics can replace the simple v1 decision frame later.",
            3,
        ),
        ModuleProfile(
            "be_lab_service",
...
BUNDLE_DEFINITIONS = {
    ...
    "Computer Vision Program": {
        "purpose": "Production Computer Vision frame: Modbus trigger/result I/O, stored Master Sample with ROI/stations and localization methods, plus Global whole-image and Local station Lab Service orchestration.",
        "minimal": (
        ...
        "deep": (
            ...
            ("FE", "docs/llm_context/updates/v0700_computer_vision_frame.md"),
        ),
    },
    "Lab Service": {
...
def render_architecture(
    ...
    lines = [
        ...
        "  → Master Sample: stored image + ROI/stations + Manual/Blur Template/Fourier Phase locator",
        "  → Working Global: full-image Lab Service bindings",
        "  → Working Local: located ROI crop = station → per-station Lab Service bindings",
        "  → simple v1 decision frame → overall OK/NG",
        "```",
        "",
        "### Lab Service",
        "",
